...
QImage GDSIIConverter::Convert(const IntensityField &f)
{
    if(f.GetRows()>0 && f.GetCols()>0)
    {
        QImage _img = QImage(static_cast<int>(f.GetCols()),static_cast<int>(f.GetRows()),QImage::Format_ARGB32_Premultiplied);
        _img.fill(Qt::white);
        double max=f.GetMax();
